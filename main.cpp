...
void eventHandler(Scorecard* scorecard, QTextEdit* gameStatusBox, const QList<Player>& teamAPlayers, const QList<Player>& teamBPlayers) {
	...
	while(pullEvent(scorecard, buffer, sizeof(buffer))) {
		...
		if(strcmp(hit, "43") == 0) {
			...
			for (auto& p: const_cast<QList<Player>&>(teamAPlayers))
				if (p.codename == taggername)
					p.basehit = true;

		}
		else if (strcmp(hit, "53") == 0) {
			output = QString("%1 tagged the Red Base").arg(taggername);
			for (auto& p: const_cast<QList<Player>&>(teamBPlayers))
				if (p.codename == taggername)
					p.basehit = true;
...
int main(int argc, char *argv[])
{
	...
	QPixmap baseIcon("images/baseicon.jpg");
	QPixmap scaledBaseIcon = baseIcon.scaled(12, 12, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
//---------------- Text Boxes ----------------
	...
	gameTeamANameList->setTextElideMode(Qt::ElideNone);
	gameTeamBNameList->setTextElideMode(Qt::ElideNone);
...
//---------------- Timer Logic ----------------
    ...
    endGameRoutine = [&]()
    {
		...
		for (auto& p : teamAPlayers) p.basehit = false;
		for (auto& p : teamBPlayers) p.basehit = false;
    ...
    updateTeamDisplays = [&]()
    {
        ...
        for (const auto& p : teamAPlayers)
        {
			...
			if (p.basehit) {
				nameItem->setIcon(scaledBaseIcon);
				nameItem->setSizeHint(QSize(200, 22));
			}
